...
$$;
SQL
sudo -u postgres psql -d postgres -v ON_ERROR_STOP=1 -c \"SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname IN ('n8n','bridge-intdatadb') AND pid <> pg_backend_pid();\"
sudo -u postgres psql -d postgres -v ON_ERROR_STOP=1 <<'SQL'
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'intbridge') AND NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'intnexus') THEN
    ALTER ROLE intbridge RENAME TO intnexus;
  END IF;
END
$$;
SQL
...
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_database WHERE datname = 'bridge-intdatadb') AND NOT EXISTS (SELECT 1 FROM pg_database WHERE datname = 'intnexusdb') THEN
    ALTER DATABASE "bridge-intdatadb" RENAME TO intnexusdb;
  END IF;
END
$$;
SQL
docker rm -f n8n >/dev/null 2>&1 || true
docker image rm n8nio/n8n:latest >/dev/null 2>&1 || true
sudo install -d -m 0775 -o deploy -g devs /var/lib/intnexus/sessions /var/lib/intnexus-test/sessions
...
Path(r'D:\int\tools\.tmp\20260424T161500\stdout.txt').write_bytes(r.stdout)
Path(r'D:\int\tools\.tmp\20260424T161500\stderr.txt').write_bytes(r.stderr)
print(r.returncode)
print(r'D:\int\tools\.tmp\20260424T161500\stdout.txt')
print(r'D:\int\tools\.tmp\20260424T161500\stderr.txt')
