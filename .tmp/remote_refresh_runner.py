...
. /int/data/.env
set +a
docker exec -i -e PGPASSWORD="{secret}" multica-postgres-1 psql -h "$DB_HOST" -p "$DB_PORT" -U agents -d "$POSTGRES_DB" -v ON_ERROR_STOP=1 -f /dev/stdin < /tmp/punktb-prod-dev-refresh/target_refresh_dry.sql
'''
