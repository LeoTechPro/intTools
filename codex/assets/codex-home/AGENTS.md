...
## Запуски и кэши
- Любые команды, создающие tool cache или временные артефакты, запускай из корня целевого проекта, а не из произвольного каталога внутри `/git`.
- Cross-repo запуск тестов и линтеров по абсолютным путям из чужого `cwd` запрещён, если он может выбрать `rootdir=/git` и оставить кэш на верхнем уровне дерева.
- Если такой запуск неизбежен, явно задавай target root и cache dir в project-local ignored path или `/git/.tmp/<project>`, но не в корне `/git`.
