...
- `git pull --ff-only` на чистой ветке с валидным upstream выполняй как обычный pre-work sync без отдельного разрешения владельца; если дерево грязное, upstream отсутствует или fast-forward не проходит, остановись и запроси дальнейшие действия.
