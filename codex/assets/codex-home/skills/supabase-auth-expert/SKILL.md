...
description: 'Supabase Auth: регистрация/вход, управление пользователями, admin-операции
  и восстановление.'
---
...
This skill provides authentication and user management operations through the Supabase Auth API. Supports email/password authentication, session management, user metadata, admin operations, and password recovery.
...
**Helper script:**
This skill uses a local Supabase API helper (included in this skill). Make sure to source it:
```bash
source "$(dirname "${BASH_SOURCE[0]}")/supabase-api.sh"
```
