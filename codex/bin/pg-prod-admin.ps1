...
switch ($PSCmdlet.ParameterSetName) {
    ...
    "Sql" {
        & $intdb sql --profile $profile --sql $Sql --write --approve-target $profile --force-prod-write
        exit $LASTEXITCODE
    }
    "File" {
        & $intdb file --profile $profile --path $Path --write --approve-target $profile --force-prod-write
        exit $LASTEXITCODE
    }
}
