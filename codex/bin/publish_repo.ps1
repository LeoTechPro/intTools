...
$sshResolverPath = Join-Path $PSScriptRoot "int_ssh_resolve.ps1"
if (-not (Test-Path $sshResolverPath)) {
    throw "ssh resolver not found: $sshResolverPath"
}
. $sshResolverPath
...
function Invoke-SshChecked {
    ...
    $sshArgs = @($SshTarget.SshArgs)
    $displayTarget = [string]$SshTarget.Destination
    $output = (& ssh @sshArgs $Command 2>&1 | Out-String).Trim()
    if ($LASTEXITCODE -ne 0) {
        $suffix = if ([string]::IsNullOrWhiteSpace($output)) { "" } else { ": $output" }
        throw "ssh $displayTarget failed$suffix"
    }
}
...
try {
    ...
    if (-not $NoDeploy -and $DeployMode -ne "none") {
        if ($DeployMode -eq "ssh-fast-forward") {
            ...
            $sshTarget = Resolve-IntSshTarget -RequestedHost $DeployHost
            $sshCommand = "cd $DeployRepoPath && git fetch --prune origin $DeployFetchRef && git pull --ff-only origin $DeployPullRef"
            Invoke-SshChecked -SshTarget $sshTarget -Command $sshCommand
            $transport = [string]$sshTarget.Transport
            $fallbackInfo = if ([bool]$sshTarget.FallbackUsed) { ", fallback=public" } else { "" }
            $actions.Add("${resolvedRepoName}: deployed via ssh-fast-forward transport=$transport$fallbackInfo to $([string]$sshTarget.Destination):${DeployRepoPath}")
