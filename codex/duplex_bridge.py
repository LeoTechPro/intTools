...
def force_back_to_topic(tag_from: str) -> str:
    return (
        f"{tag_from}: Стоп. Ты ушёл от темы. Немедленно вернись к анализу /int/data и /int/nexus. "
...
async def main():
    ...
    RULES = (
        "Требования: 1) анализ ТОЛЬКО /int/data и /int/nexus; 2) без 'планирую/готовлюсь'; "
