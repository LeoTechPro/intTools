...
- MCP-first routing: use `mcp__intdata_control__lockctl_*` raw tools when available; if they are not visible, run `tool_search` for `lockctl` before any shell `lockctl` fallback.
- Shell fallback is degraded mode only, because Codex shell sandbox may not have access to runtime lock storage.
