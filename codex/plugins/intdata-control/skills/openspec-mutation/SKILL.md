...
- Режим: mutating
- Approval / issue requirements: Для mutating/high-risk вызова требуются owner approval, `confirm_mutation=true` и `issue_context=#N`; unattended mutation запрещена.
...
- Пример вызова: `{"name":"openspec_archive","arguments":{"confirm_mutation": true, "issue_context": "#800", "change_name": "agent-tool-plane-v1"}}`
...
- Пример вызова: `{"name":"openspec_change_mutate","arguments":{"confirm_mutation": true, "issue_context": "#800", "subcommand": "set", "args": ["agent-tool-plane-v1"]}}`
- Fallback/blocker: если required args неизвестны, MCP вернул policy/config error, или запрос требует mutation без approval, остановиться и записать blocker вместо shell fallback.
...
- Пример вызова: `{"name":"openspec_spec_mutate","arguments":{"confirm_mutation": true, "issue_context": "#800", "subcommand": "set", "args": ["process"]}}`
- Fallback/blocker: если required args неизвестны, MCP вернул policy/config error, или запрос требует mutation без approval, остановиться и записать blocker вместо shell fallback.

### openspec_new
- Когда: нужно создать новый OpenSpec change уровня `delta` или `full` после owner approval.
- Required inputs: `confirm_mutation`, `issue_context`, `spec_level`, `args`
- Optional/schema inputs: `cwd`, `timeout_sec`
- Режим: mutating
- Approval / issue requirements: Для mutating/high-risk вызова требуются owner approval, `confirm_mutation=true` и `issue_context=#N`; unattended mutation запрещена.
- Не использовать когда: Goal Contract задаёт `Specification level: none`, задача ещё в PLAN/аудите или нет согласованного scope.
- Пример вызова: `{"name":"openspec_new","arguments":{"confirm_mutation": true, "issue_context": "#801", "spec_level": "delta", "args": ["change", "issue-801-adjust-api-contract"]}}`
- Fallback/blocker: если required args неизвестны, MCP вернул policy/config error, или запрос требует mutation без approval, остановиться и записать blocker вместо shell fallback.
...
- Пример вызова: `{"name":"openspec_exec_mutate","arguments":{"confirm_mutation": true, "issue_context": "#800", "args": ["archive", "agent-tool-plane-v1"]}}`
- Fallback/blocker: если required args неизвестны, MCP вернул policy/config error, или запрос требует mutation без approval, остановиться и записать blocker вместо shell fallback.
