...
### openspec_validate
- Когда: нужно проверить change/spec/catalog или профиль `delta|full`.
- Required inputs: нет
- Optional/schema inputs: `cwd`, `timeout_sec`, `item`, `strict`, `spec_level`
- Режим: read-only
- Approval / issue requirements: Не требуется для read-only вызова. Если команда превращается в запись, остановиться и получить owner approval.
- Не использовать когда: нужна mutation. Для profile validation передавай одновременно `item=issue-N-slug` и `spec_level`.
- Пример вызова: `{"name":"openspec_validate","arguments":{"cwd":"/int","item":"issue-801-risk-based-openspec","strict":true,"spec_level":"full"}}`
- Fallback/blocker: если required args неизвестны, MCP вернул policy/config error, или запрос требует mutation без approval, остановиться и записать blocker вместо shell fallback.
