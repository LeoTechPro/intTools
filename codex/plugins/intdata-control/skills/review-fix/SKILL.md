...
- Для `/int/tools` перед tracked-правками соблюдайте active OpenSpec scope, Multica issue gate, `sync_gate_start` и `lockctl` locks; перед закрытием проверяйте `sync_gate_finish`, если он применим. Если gate/tool недоступен, фиксируйте blocker.
