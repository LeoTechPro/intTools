...
- Для `/int/tools` перед tracked-правками соблюдайте active OpenSpec scope, Multica issue gate и `lockctl` locks. Git status/fetch/pull/push выполняйте явными native git commands; removed/forbidden `int_git_sync_gate`/`sync_gate_*` не использовать.
