...
- Для `/int/tools` перед tracked-правками соблюдайте active OpenSpec scope, GitHub `INT-*` issue gate и `coordctl` session/intent leases. Git status/fetch/pull/push выполняйте явными native git commands; removed/forbidden legacy coordination/sync-gate tools не использовать.
