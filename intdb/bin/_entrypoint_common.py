...
INTDB_ROOT = Path(__file__).resolve().parents[1]
INTDB_PY = INTDB_ROOT / "lib" / "intdb.py"
...
def _run_intdb(args: Sequence[str]) -> int:
    if not INTDB_PY.exists():
        raise WrapperError(f"intdb core not found: {INTDB_PY}")
    command = [sys.executable, str(INTDB_PY), *args]
...
def readonly_main(config: EntryPointConfig) -> int:
    ...
    if args.doctor:
        return _run_intdb(["doctor", "--profile", config.profile])
    if args.sql:
        return _run_intdb(["sql", "--profile", config.profile, "--sql", args.sql])
    return _run_intdb(["file", "--profile", config.profile, "--path", args.path])
