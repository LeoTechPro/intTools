...
def normalize_path(repo_root: str, raw: str) -> str:
    ...
    if re.match(r"^[A-Za-z]:", normalized):
        raise LockCtlError("INVALID_PATH", f"path must be relative or absolute under repo root: {raw}")
...
def build_parser() -> tuple[argparse.ArgumentParser, dict[str, argparse.ArgumentParser]]:
    ...
    release_path = add_command(
        ...
        description=textwrap.dedent(
            ...
            Пример:
              lockctl release-path --repo-root /int/crm --path README.md --owner codex:session-1
            """
        ).strip(),
    )
