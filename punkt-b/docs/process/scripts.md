...
- Локальный dev-контур доступен (по умолчанию `https://dev.punkt-b.pro`).

Если Playwright сообщает о нехватке системных библиотек — согласовать установку зависимостей с владельцем/DevOps.

### Запуск
```bash
BASE_URL=https://dev.punkt-b.pro \
WORKSPACE_EMAIL=manager.example@punkt-b.test \
WORKSPACE_PASSWORD='******' \
...
- `BASE_URL` (default `https://dev.punkt-b.pro`)
...
- `REPORT_PATH` (default `~/.codex/tmp/punkt-b/agent_browser_role_smoke_<UTC>.json`)
...
- `REPORT_PATH` (default `~/.codex/tmp/punkt-b/agent_browser_key_scenarios_<UTC>.json`)
...
SPECIALIST_EMAIL='specialist.demo@punkt-b.test' \
CLIENT_EMAIL='client.demo@punkt-b.test' \
PUBLIC_CLIENT_EMAIL='public.demo@punkt-b.test' \
bash ops/agent-browser/agent_browser_key_scenarios.sh
```
...
- `REPORT_PREFIX` (default `~/.codex/tmp/punkt-b/agent_browser_gate_<UTC>`)
...
### Запуск
```bash
API_BASE_URL=https://api-dev.punkt-b.pro \
SERVICE_ROLE_KEY='******' \
...
- обрабатывает только явные заявки `~/.codex/tmp/punkt-b/autopush/*.env`;
...
- owner override возможен только при `PUNKTB_DOCS_OWNER_APPROVED=YES` и флаге `--allow-owner-override`.
...
- `teamlead_orchestrator.sh --mode milestone` обязателен только для `--full` major/risky scope; при `retry_required=true` повторяет весь finish-loop до лимита `PUNKTB_FINISH_LOOP_MAX`;
...
- формирует временный message-файл в `~/.codex/tmp/punkt-b` (с trace-header) и гарантирует `Refs #<id>` через `lock_issue_resolver.py ensure-message`;
...
- пишет артефакты в `~/.codex/tmp/punkt-b/autoreview/<issue>/<timestamp>/`, итоговый summary лежит в `final-gate.json`;
...
- требует `PUNKTB_MAIN_PUSH_APPROVED=YES`;
...
### Запуск
```bash
PUNKTB_MAIN_PUSH_APPROVED=YES bash ops/release/release_main.sh --issue 1205
```
