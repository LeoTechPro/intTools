...
- Для проекта PunctB БД `77.95.201.51:5432/PunctBPro` и DSN `<REDACTED_DSN_PUNCTBPRO>` считаются prod БД `punctb.pro`.
...
- БД `77.95.201.51:5432/PunktB` и `77.95.201.51:5432/PunctB`, а также DSN `<REDACTED_LEGACY_DSN_PUNKB>` считаются запрещёнными к изменениям legacy-контурами старой платформы.
